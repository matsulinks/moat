...
### FR-8: AI仲裁エージェント
- 新ソフトデプロイ時のセキュリティ衝突を自動検知
- OpenAI APIで問題を分析・解説（非技術者向け説明）
- 開発AI へGitHub Issue作成またはクリップボードコピーで報告（ユーザー設定）
- 全送信前に人間承認必須、個人情報は匿名化
- 外部通信先: api.openai.com, api.github.com のみ

### FR-9: 脅威インテリジェンス・ワクチン機能
- ホワイトリストURL（CVE/NVD, Falcoコミュニティルール）から脅威情報を定期取得
- 取得データはSHA-256ハッシュ検証必須
- AIが新脅威を分析し、Falco/iptablesルールを自動生成（ワクチン）
- 生成ルールはステージング検証後、人間承認を経て本番適用
- コミュニティ共有はデフォルトOFF、有効化時も完全匿名化
...
- **Should have**: YubiKey/Vault統合、自動ローテーション、Falco syscall監視、
  Prometheusダッシュボード、AI仲裁エージェント（衝突検知+報告）
- **Could have**: PromptGuard自動統合、週次audit自動レポート、脅威インテリジェンス・ワクチン機能
- **Won't have (Phase 1)**: 公開Web UI、自動スキルレビューAI、federated multi-user、コミュニティ脅威共有
